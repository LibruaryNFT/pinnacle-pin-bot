 ...
 * - FIX: Row 3 split into two lines so serial never gets cut off
 * - Logo removed
 */
...
async function renderSaleCard({
  ...
  const INFO_H = 260 * SCALE;      // 520px info bar — extra room for two-line metadata
  const IMG_H = 480 * SCALE;       // 960px for image area
  const FADE_H = 150 * SCALE;      // tall gradient blend
  const H = IMG_H + INFO_H;        // 1440px total
  const PAD = 28 * SCALE;
  ...
  /* ── Row 3: Set name (full width, own line) ── */
  ctx.font = `600 ${fs(17)}px ${SG}`;
  ctx.fillStyle = SECONDARY;
  ctx.fillText(truncate(ctx, setName, W - PAD * 2), PAD, y + 14 * SCALE);

  y += 32 * SCALE;

  /* ── Row 4: Edition badge + Chaser + Variant + Serial ── */
  let x = PAD;
